...
*@note Construction code for derived classes and objects is executed on the ''JavaFX event-dispatch thread''. To achieve
this, the execution of such code must be delayed until after the ''JavaFX'' `javafx.application.Application` instance
has been created and initialized. However, the construction code for traits that extend this trait will be executed upon
construction, on the current thread, in the normal manner. This functionality is provided by the [[DelayedInit]] trait.
*/
 ...
 *
  ...
  *@note [[DelayedInit]] causes the compiler to wrap `class` and `object` constructor code in a call to this function.
